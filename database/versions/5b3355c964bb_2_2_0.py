...
def fix_table_sequence(connection, table_name):
    ...
    try:
        ...
        if is_identity == 'YES' or (column_default and 'GENERATED BY DEFAULT AS IDENTITY' in column_default):
            logger.info(f"表 {table_name} 的id列已经是Identity类型，检查是否需要重置序列")
            reset_identity_sequence(connection, table_name)
            return
...
def reset_identity_sequence(connection, table_name):
    """
    重置已存在的Identity序列，设置为当前最大值+1
    """
    try:
        # 获取当前序列的最大值
        result = connection.execute(sa.text(f"""
        ...
        # 检查表中是否有数据
        count_result = connection.execute(sa.text(f"""
        ...
        # 如果表中没有数据，不需要重置序列
        if row_count == 0:
            logger.debug(f"表 {table_name} 没有数据，无需重置序列")
            return
        
        # 重置Identity序列的当前值
        connection.execute(sa.text(f"""
            ALTER TABLE "{table_name}" 
            ALTER COLUMN id RESTART WITH {next_value}
        """))

        logger.info(f"表 {table_name} Identity序列已重置，起始值为 {next_value}")

    except Exception as e:
        logger.error(f"重置表 {table_name} Identity序列时出错: {e}")
        raise
