...
# 插件远程静态资源路径的版本命名空间前缀
PLUGIN_VERSION_URL_PREFIX = "__versions__"
...
def resolve_instance_version_dir(
    ...
    """
    desired_version = (
        None if instance is None else instance.pinned_version
    )
    try:
...
def recycle_plugin_version_directories(
    ...
    *,
    ...
    :param has_pending_installation: 是否存在该插件未收尾的安装 journal。安装事务
        的载荷收据覆盖整个插件根目录；事务未收尾时删除任一旧版本会改变收据，使
        COMMITTED 恢复无法证明数据库与磁盘属于同一载荷，因此直接跳过本次回收。
    ...
    def recent_rank(version: str) -> tuple[str, Any]:
        """先按登记时间排序，登记时间缺失或并列时再按版本号语义序兜底。

        清单丢失或损坏时所有登记时间都读成空串，只按它排序会让键全部相等；稳定
        排序不反转等值元素，窗口会退化成保留目录名字典序最靠前的几个（也就是
        最旧的几个），把当前正在加载的最新版本删掉。
        """
        return installed_at(version), cmp_to_key(_version_order)(version)

    recent_window = set(sorted(on_disk, key=recent_rank, reverse=True)[: max(retention, 0)])
