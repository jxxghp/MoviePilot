...
class PluginDefaultTargetControl:
    ...
    随机挑一个——那会让同一次调用在不同时刻落到不同实例，行为不可复现，用户
    也无从知道刚才究竟是哪个实例执行的。同理，默认目标停用时也不静默改走另
    一个正在运行的实例：那等于用户停用了一个实例、调用却被悄悄改道，且不留
    任何痕迹。只有本体、没有任何分身的插件不受这套机制约束，直接使用本体，
    不要求显式设置默认目标，单实例场景不应被打扰。
    """
    ...
    def _candidates(self, plugin_id: str) -> list[PluginCallCandidate]:
        ...
        return [
            PluginCallCandidate(
                ...
                is_running=self._is_enabled(running.get(instance.instance_id)),
    ...
    def _is_enabled(plugin: Any) -> bool:
        ...
        已加载不等于已启用：用户在界面上关掉启用开关只会重新 ``init_plugin``，
        实例仍然留在运行表里。只看运行表会把停用实例当成可用的调用目标选中，
        随后在调用点以「插件不存在」收场；而这套机制的语义是停用就直接报错、
        要求显式指定实例。取不到状态时按可用处理，不因判据缺失打断调用。
        """
        if plugin is None:
            return False
        get_state = getattr(plugin, "get_state", None)
        if not callable(get_state):
            return True
        try:
            return bool(get_state())
        except Exception:  # noqa: BLE001 - 单个实例取状态失败不能影响整体裁决
            return False
    ...
    def set_target(self, plugin_id: str, instance_id: str) -> bool:
        ...
        登记过任何设置时，先落盘一条默认视图的本体记录，确保随后的数据库级
        清旧置新有行可操作——这与日志等级那处对本体的写入语义一致。
        ...
        if self._get_instance(plugin_id) is not None:
            raise LookupError(f"{plugin_id} 是分身实例，请使用源插件 ID 设置默认调用目标")
    ...
    def clear_target(self, plugin_id: str, instance_id: str) -> None:
        ...
        if not self._plugin_exists(plugin_id):
            raise LookupError(f"插件 {plugin_id} 不存在")
        if self._get_instance(plugin_id) is not None:
            raise LookupError(f"{plugin_id} 是分身实例，请使用源插件 ID 清除默认调用目标")
        current = next(
