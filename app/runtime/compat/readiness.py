...
from app.runtime.compat.resources import _constant_dynamic_import, _dynamic_import_aliases
...
def _classify_plugin_module(module_name: str, own_plugin_id: str) -> tuple[str, str] | None:
    ...
    :return: 分类为 ``self`` 表示指向 own_plugin_id 自身，``cross`` 表示指向其它
        插件；module_name 不属于 ``app.plugins.<pid>`` 形态时返回 None
    """
...
def plugin_multi_version_blockers(plugin_id: str, source_dirs: Iterable[Path]) -> list[str]:
    ...
    自引用绝对 import 在版本化目录下必然 ``ModuleNotFoundError``；在宿主共享声明
    基类上定义的模型会让两个版本映射到同名表，第二个版本 import 时直接冲突。这
    两类都是本插件自身的写法错误，会在真正双版本并存时必然失败，因此纳入阻断；
    跨插件依赖不是本插件自身的写法错误，不纳入阻断。
