...
class Subscribe(Base):
    ...
    # 缺失集数
    lack_episode = mapped_column(Integer)
    # 附加信息
    note = mapped_column(JSON)
    # 状态：N-新建 R-订阅中 P-待定 S-暂停
    state = mapped_column(String, nullable=False, index=True, default='N')
    # 最后更新时间
    last_update = mapped_column(String)
    # 创建时间
    date = mapped_column(String)
    # 订阅用户
    username = mapped_column(String, index=True)
    # 订阅站点
    sites = mapped_column(JSON, default=list)
    # 下载器
    ...
    # 是否手动修改过总集数 0否 1是
    manual_total_episode = mapped_column(Integer, default=0)
    # 自定义识别词
