...
class NotificationHelper(ServiceBaseHelper[NotificationConf]):
    ...
    def _is_notification_service(self, service_type: str, instance_type: Optional[Type] = None,
                                 service: Optional[ServiceInfo] = None, name: Optional[str] = None,
                                 instance: Optional[
                                     Union[WeChat, VoceChat, Telegram, SynologyChat, Slack]] = None) -> bool:
        """
        通用的消息通知服务类型判断方法

        :param service_type: 消息通知服务的类型名称（如 'wechat', 'voicechat', 'telegram', 等）
        :param instance_type: 实例对象的类型（如 WeChat, VoceChat, Telegram）
        ...
        if instance_type and isinstance(instance, instance_type):
            return True
    ...
    def is_wechat(self, service: Optional[ServiceInfo] = None, name: Optional[str] = None,
                  instance: Optional[WeChat] = None) -> bool:
        """
        判断指定的消息通知服务是否为 WeChat 类型，需要传入 `service`、`name` 或 `instance` 中的任一参数
    ...
    def is_voicechat(self, service: Optional[ServiceInfo] = None, name: Optional[str] = None,
                     instance: Optional[VoceChat] = None) -> bool:
        """
        判断指定的消息通知服务是否为 VoceChat 类型，需要传入 `service`、`name` 或 `instance` 中的任一参数
    ...
    def is_telegram(self, service: Optional[ServiceInfo] = None, name: Optional[str] = None,
                    instance: Optional[Telegram] = None) -> bool:
        """
        判断指定的消息通知服务是否为 Telegram 类型，需要传入 `service`、`name` 或 `instance` 中的任一参数
    ...
    def is_synologychat(self, service: Optional[ServiceInfo] = None, name: Optional[str] = None,
                        instance: Optional[SynologyChat] = None) -> bool:
        """
        判断指定的消息通知服务是否为 SynologyChat 类型，需要传入 `service`、`name` 或 `instance` 中的任一参数
    ...
    def is_slack(self, service: Optional[ServiceInfo] = None, name: Optional[str] = None,
                 instance: Optional[Slack] = None) -> bool:
        """
        判断指定的消息通知服务是否为 Slack 类型，需要传入 `service`、`name` 或 `instance` 中的任一参数
