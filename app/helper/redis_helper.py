...
class RedisHelper:
    ...
    def _init_cache_backend(self):
        ...
        if cache_type == "redis":
            ...
            if redis_url:
                try:
                    logger.debug(f"Attempting to use RedisBackend with URL: {redis_url}")
                    self._cache_backend = RedisBackend(redis_url=redis_url, ttl=1800)
                except RuntimeError:
                    logger.warning("Falling back to CacheToolsBackend due to Redis connection failure.")
                    self._cache_backend = CacheToolsBackend(maxsize=512, ttl=1800)
            else:
            ...
            # 如果不是 Redis，使用内存缓存
            logger.debug(f"Using CacheToolsBackend with default maxsize: 512, TTL: 1800")
            self._cache_backend = CacheToolsBackend(maxsize=512, ttl=1800)
    ...
    def set(self, key: str, value: Any, ttl: Optional[int] = None, 
        ...
        if self._cache_backend:
            self._cache_backend.set(key, value, ttl or 1800, region, **kwargs)
    ...
    def get(self, key: str, region: Optional[str] = DEFAULT_CACHE_REGION) -> Any:
        ...
        """
        if self._cache_backend:
            return self._cache_backend.get(key, region)
        return None
    ...
    def exists(self, key: str, region: Optional[str] = DEFAULT_CACHE_REGION) -> bool:
        ...
        """
        if self._cache_backend:
            return self._cache_backend.exists(key, region)
        return False
    ...
    def delete(self, key: str, region: Optional[str] = DEFAULT_CACHE_REGION) -> None:
        ...
        """
        if self._cache_backend:
            self._cache_backend.delete(key, region)
    ...
    def clear(self, region: Optional[str] = None) -> None:
        ...
        """
        if self._cache_backend:
            self._cache_backend.clear(region)
    ...
    def get_cache_key(self, func, args, kwargs):
        ...
        获取缓存键
        """
        return CacheBackend.get_cache_key(func, args, kwargs)
...
def cached(region: Optional[str] = None, maxsize: Optional[int] = 512, ttl: Optional[int] = 1800,
    ...
    def is_valid_cache_value(cache_key: str, cached_value: Any, cache_region: str) -> bool:
        ...
        if not skip_none and cached_value is None:
            if not redis_helper.exists(key=cache_key, region=cache_region):
                return False
        return True
    ...
    def decorator(func):
        ...
        if is_async:
            ...
            async def async_wrapper(*args, **kwargs):
                # 获取缓存键
                cache_key = redis_helper.get_cache_key(func, args, kwargs)
                # 尝试获取缓存
                cached_value = redis_helper.get(cache_key, region=cache_region)
                ...
                redis_helper.set(cache_key, result, ttl=ttl, maxsize=maxsize, region=cache_region)
                return result
            ...
            def cache_clear():
                ...
                清理缓存区
                """
                redis_helper.clear(region=cache_region)
...
def close_redis_cache() -> None:
    ...
    try:
        if redis_helper:
            redis_helper.close()
            logger.info("Redis cache backend closed successfully.")
    except Exception as e:
        logger.info(f"Error while closing Redis cache backend: {e}")