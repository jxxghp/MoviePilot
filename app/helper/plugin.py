...
class PluginHelper(metaclass=Singleton):
    ...
    def __pip_uninstall_and_install_with_fallback(requirements_file: Path) -> Tuple[bool, str]:
        ...
        if settings.PIP_PROXY:
            strategies.append(("镜像站",
                               [str(Path(settings.ROOT_PATH).parent.parent.joinpath('Python3.11').joinpath('python')), "-m",
        ...
        strategies.append(("直连", [str(Path(settings.ROOT_PATH).parent.parent.joinpath('Python3.11').joinpath('python')),
    ...
    def __pip_install_with_fallback(requirements_file: Path) -> Tuple[bool, str]:
        ...
        # 添加策略到列表中
        if settings.PIP_PROXY:
            strategies.append(("镜像站", [str(Path(settings.ROOT_PATH).parent.parent.joinpath('Python3.11').joinpath('python')), "-m", "pip", "install", "-r", str(requirements_file), "-i", settings.PIP_PROXY]))
        if settings.PROXY_HOST:
            strategies.append(
                ("代理", [str(Path(settings.ROOT_PATH).parent.parent.joinpath('Python3.11').joinpath('python')), "-m", "pip", "install", "-r", str(requirements_file), "--proxy", settings.PROXY_HOST]))
        strategies.append(("直连", [str(Path(settings.ROOT_PATH).parent.parent.joinpath('Python3.11').joinpath('python')), "-m", "pip", "install", "-r", str(requirements_file)]))
