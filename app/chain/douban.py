...
class DoubanChain(ChainBase, metaclass=Singleton):
    ...
    def movie_top250(self, page: int = 1, count: int = 30) -> list[MediaInfo] | None:
        """
        获取豆瓣电影TOP250
        :param page:  页码
        :param count:  每页数量
    ...
    def movie_showing(self, page: int = 1, count: int = 30) -> list[MediaInfo] | None:
        """
        获取正在上映的电影
        """
        return self.run_module("movie_showing", page=page, count=count)

    def tv_weekly_chinese(self, page: int = 1, count: int = 30) -> list[MediaInfo] | None:
        """
        获取本周中国剧集榜
        """
        return self.run_module("tv_weekly_chinese", page=page, count=count)

    def tv_weekly_global(self, page: int = 1, count: int = 30) -> list[MediaInfo] | None:
        """
        获取本周全球剧集榜
        """
    ...
    def douban_discover(self, mtype: MediaType, sort: str, tags: str,
                        page: int = 0, count: int = 30) -> list[MediaInfo] | None:
        """
        发现豆瓣电影、剧集
        :param mtype:  媒体类型
        :param sort:  排序方式
    ...
    def tv_animation(self, page: int = 1, count: int = 30) -> list[MediaInfo] | None:
        """
        获取动画剧集
        """
        return self.run_module("tv_animation", page=page, count=count)

    def movie_hot(self, page: int = 1, count: int = 30) -> list[MediaInfo] | None:
        """
        获取热门电影
        """
        return self.run_module("movie_hot", page=page, count=count)

    def tv_hot(self, page: int = 1, count: int = 30) -> list[MediaInfo] | None:
        """
        获取热门剧集
        """
        return self.run_module("tv_hot", page=page, count=count)

    def movie_credits(self, doubanid: str) -> list[schemas.MediaPerson] | None:
        """
        根据TMDBID查询电影演职人员
        :param doubanid:  豆瓣ID
        """
    ...
    def tv_credits(self, doubanid: str) -> list[schemas.MediaPerson] | None:
        """
        根据TMDBID查询电视剧演职人员
        :param doubanid:  豆瓣ID
        """
