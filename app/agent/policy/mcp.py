...
OPERATION_DESCRIPTIONS = {
    ...
    "download.artist_collection": "Submit one artist-wide music torrent to MoviePilot's collection download workflow.",
    ...
    "media.cache.get": "Inspect the administrator-only TMDB recognition cache and summary counts.",
    "media.cache.delete": "Delete one administrator-only TMDB recognition-cache entry by exact key.",
    "media.cache.clear": "Clear the complete administrator-only TMDB recognition cache.",
    ...
    "site.cookie.set": "Persist a browser-obtained authentication cookie and optional User-Agent for one site without replacing other settings.",
    ...
    "media.classification.fields": "Read the media-classification field catalog and source capabilities.",
    "media.classification.policy.get": "Read the active automatic media-classification policy.",
    "media.classification.policy.update": "Validate and publish a complete automatic media-classification policy.",
    "media.classification.policy.validate": "Validate an unpublished automatic media-classification policy without saving it.",
    "media.classification.policy.preview": "Preview automatic media classification for selected media or normalized facts.",
    "media.classification.policy.impact": "Estimate how an unpublished media-classification policy changes bounded recent samples.",
    "media.classification.policy.history": "Read the bounded history of published media-classification policies.",
    "media.classification.policy.rollback": "Publish a selected historical media-classification policy as a new revision.",
    ...
    "subscription.execution.list": "List recent durable subscription-search batches visible to the current user.",
    "subscription.execution.get": "Read one durable subscription-search batch visible to the current user.",
    "subscription.execution.cancel": "Request cancellation of one durable subscription-search batch before its download side-effect boundary.",
    ...
    "system.module.list": "List enabled MoviePilot module IDs and localized names.",
    ...
    "plugin.instance.set_enabled": "Enable or disable one plugin instance, host or clone; disabling only stops it running and keeps its configuration and pinned version for a later re-enable.",
    "plugin.instance.purge": "Irreversibly delete the selected scopes of one plugin instance's configuration and data; a clone's instance record goes with it, a host's record is kept.",
}
...
FIELD_DESCRIPTIONS = {
    ...
    "artist_name": "Artist name preserved as the identity label for an artist-wide music collection.",
    ...
    "cascade": "Uninstall every clone of the source plugin along with the plugin itself; off by default.",
    "auto_paused": "Whether MoviePilot paused further automatic retries for this transfer history entry.",
    "cleanup_error": "Downloader cleanup error message; cleanup is tracked separately from the media transfer result.",
    "cleanup_status": "Downloader cleanup state for this history entry, independent of the media transfer result.",
    "category": "MoviePilot media category or filter-group category, depending on the operation.",
    "cache_key": "Exact recognition-cache key returned by the corresponding recognition-cache get operation.",
    ...
    "expected_revision": "Current revision from a preceding read, used to reject concurrent state changes.",
    ...
    "failure_stage": "User-actionable failure stage derived from the error message and retry count.",
    ...
    "enabled": "Target enabled state; false stops the instance while keeping its configuration and pinned version.",
    "config": "Whether to delete this instance's business parameters.",
    "plugin_data": "Whether to delete the rows this instance wrote to the plugin data table.",
    "own_database": "Whether to destroy this instance's dedicated database.",
    "data_directory": "Whether to delete this instance's whole directory under the plugin data path; selecting it always destroys the dedicated database first.",
    ...
    "labels": "Labels attached to the media, classification result, or torrent result.",
    ...
    "limit": "Maximum number of recent subscription-search batches to return.",
    ...
    "music_release_regions": "MusicBrainz release-region priority for this manual transfer; empty inherits the system setting.",
    "music_release_scripts": "MusicBrainz release-script priority for this manual transfer; empty inherits the system setting.",
    ...
    "pinned_version": "Plugin version this instance is pinned to; empty means it follows the plugin's current version.",
    ...
    "recovery_action": "Suggested recovery action derived from the failure stage of this transfer history entry.",
    "restore_previous": "Reuse the configuration and data left by a previously removed clone with the same suffix; false discards them first.",
    "retry_count": "How many times this transfer history entry has been retried.",
    "retry_exhausted": "Whether this transfer history entry has used up its automatic retry budget.",
    ...
    "skip_success": "Skip history entries that already succeeded; takes precedence over reorganize and applies to preview and execution alike.",
...
FIELD_DESCRIPTIONS.update(
    {
        "all": "Child conditions that must all match.",
        ...
        "any": "Child conditions where at least one condition must match.",
        ...
        "auto_paused": "Whether a failed transfer automatically paused the related subscription.",
        ...
        "categories": "Complete ordered media-category definitions in the classification policy.",
        "category_id": "Stable media-category ID targeted by a classification rule or selection.",
        "companies": "Production companies or studios associated with the media.",
        "concurrency_key": "Workflow expression used to serialize actions sharing the same runtime key.",
        "content_rating": "Content rating assigned to the media.",
        "countries": "Normalized country or region codes used by classification rules.",
        ...
        "cleanup_error": "Error recorded while cleaning up a transfer source or destination.",
        "cleanup_status": "Current status of transfer-file cleanup after an operation.",
        ...
        "enrichment_mode": "Metadata enrichment mode used to populate classification facts.",
        "errors": "Workflow execution errors keyed or ordered by action identity.",
        "enabled": "Whether this category or classification rule participates in evaluation.",
        "entity_type": "Music entity type used by classification rules.",
        "example_limit": "Maximum number of representative impact examples to return.",
        ...
        "failure_stage": "Transfer or workflow stage at which the operation failed.",
        "fallbacks": "Fallback category or label actions used when no classification rule matches.",
        "facts": "Normalized media facts to evaluate during a classification preview.",
        "field": "Normalized classification fact field referenced by a condition.",
        "field_aliases": "Optional aliases mapping source-specific fields to normalized classification fields.",
        "field_sources": "Source provenance for normalized classification facts.",
        ...
        "extensions": "Additional normalized classification facts supplied by extensions.",
        "genre_keys": "Normalized MoviePilot genre keys used by classification rules.",
        "genre_names": "Source-provided genre names used as classification facts.",
        "genres": "Normalized music genre values used by classification rules.",
        ...
        "identity": "Stable source-native media identity used by classification facts.",
        ...
        "input": "Media or normalized-facts input used for a classification preview.",
        "album_type": "Music album or release-group type used by classification rules.",
        "artist_country": "Country or region associated with the music artist.",
        "artists": "Music artist names associated with the classified entity.",
        "adult": "Whether the media is marked as adult content.",
        ...
        "language": "Normalized media language code used by classification rules.",
        "last_time": "Timestamp of the workflow's most recent execution.",
        "kind": "Classification rule or preview-input kind selected by the request.",
        ...
        "media": "Media metadata input used for a classification preview.",
        "media_types": "Media types to which a classification rule applies.",
        ...
        "not": "Child condition group that must not match.",
        "operator": "Comparison operator used to evaluate one classification condition.",
        ...
        "policy": "Complete or draft automatic media-classification policy.",
        ...
        "provider_id": "Provider-native identifier that supplied a classification fact.",
        "provider_name": "Human-readable provider name that supplied a classification fact.",
        "production_countries": "Production-country codes matched by an automatic category rule.",
        "priority": "Classification rule ordering priority persisted by the server.",
        ...
        "recovery_action": "Recovery action reported for a failed or partially completed transfer.",
        "release_year": "Release year matched by an automatic category rule.",
        "result": "Persisted workflow action result value.",
        "revision": "Published classification policy revision or expected revision number.",
        ...
        "secondary_types": "Secondary music release-group types used by classification rules.",
        "sample_limit": "Maximum number of recent records or samples to inspect for impact analysis.",
        "samples": "Explicit normalized fact samples used for classification impact analysis.",
        "schema_version": "Classification policy schema version expected by the server.",
        "sources": "Metadata source IDs to which a classification rule applies; empty means all sources.",
        ...
        "skip_success": "Skip files already recorded as successfully organized.",
        "site_url": "Configured site URL or hostname used to select one site's statistics.",
        "updated_at": "Timestamp when the persisted object or execution state was last updated.",
        ...
        "target": "Classification rule output containing a category ID and optional labels.",
        ...
        "when": "Condition group evaluated before a classification rule target is applied.",
...
MODEL_DESCRIPTIONS = {
    ...
    "Body_download_artist_collection_api_v1_download_artist_collection_post": (
        "Artist-wide music collection download submission request."
    ),
    ...
    "PluginInstanceEnabledRequest": "Target enabled state for one plugin instance; disabling only stops it running and keeps every setting.",
    "PluginInstanceLogLevelUpdateRequest": "One plugin instance's log-level override update request.",
    "PluginInstancePurgeRequest": "Scopes to irreversibly delete when purging one plugin instance's configuration and data.",
    ...
    "SiteCookieSet": "Browser-obtained site authentication cookie and optional User-Agent to persist without changing other site settings.",
...
MODEL_DESCRIPTIONS.update(
    {
        "ClassificationCategory": "Stable media-category definition with its library path and default labels.",
        "ClassificationCondition": "One leaf condition in an automatic media-classification rule.",
        "ClassificationConditionGroup-Input": "A recursive all, any, or not condition group.",
        "ClassificationFacts": "Normalized media facts evaluated by the automatic classification policy.",
        "ClassificationFactsPreviewInput": "Normalized facts supplied directly for a classification preview.",
        "ClassificationFactSource": "Provider identity for a classification fact filled by another source.",
        "ClassificationIdentityFacts": "Stable source-native identity used by classification evaluation.",
        "ClassificationImpactRequest": "Request to compare an unpublished classification policy against the active policy.",
        "ClassificationMediaFacts": "Normalized movie, TV, or shared media facts used by classification rules.",
        "ClassificationMediaPreviewInput": "A selected media search result supplied for classification preview.",
        "ClassificationMusicFacts": "Music-specific facts used by automatic classification rules.",
        "ClassificationPolicy-Input": "Complete versioned automatic media-classification policy.",
        "ClassificationPolicyPublishRequest": "Request to validate and publish a complete classification policy with CAS revision.",
        "ClassificationPolicyRollbackRequest": "Request to publish a historical classification policy as a new revision.",
        "ClassificationPolicyValidateRequest": "Request to validate a complete classification policy without saving it.",
        "ClassificationPreviewRequest": "Request to preview a classification policy against media or normalized facts.",
        "ClassificationRule-Input": "One ordered automatic media-classification rule.",
        "ClassificationTarget": "Rule output containing an optional category ID and additional labels.",
