...
def configure_default_user_agent(user_agent: str) -> None:
    """注册宿主 User-Agent，用于识别并标记来自插件的请求。"""
...
class RequestUtils:
    ...
    def __init__(
        ...
        if headers:
            ...
            if ua and ua == _default_user_agent:
                caller_name = get_caller()
                if caller_name:
                    ua = f"{_default_user_agent} Plugin/{caller_name}"
            self._headers = {
                "User-Agent": ua,
