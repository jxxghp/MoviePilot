...
class PluginLifecycleCoordinator:
    ...
    def __init__(self) -> None:
        ...
        self._startup_token: _StartupToken | None = None
    ...
    def _try_acquire_plugin(
        self,
        plugin_id: str,
        startup_token: _StartupToken | None = None,
    ) -> bool:
    ...
    def _try_acquire_startup(self) -> _StartupToken | None:
        with self._condition:
            if self._startup_active or self._active_plugins:
                return None
            startup_token = _StartupToken()
            self._startup_active = True
            self._startup_token = startup_token
            return startup_token

    def _release_startup(self, startup_token: _StartupToken) -> None:
        with self._condition:
    ...
    @asynccontextmanager
    async def hold_startup(self) -> AsyncIterator[_StartupToken]:
