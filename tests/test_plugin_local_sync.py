...
def test_runtime_plugin_resolves_versioned_source_change(
    tmp_path,
    monkeypatch,
) -> None:
    """版本目录内的源码事件仍应解析到源插件类名，触发整棵插件树重载。"""
    plugins_root = tmp_path / "app" / "plugins"
    plugin_root = plugins_root / "demoplugin" / plugin_version_dir_name("1.0.0")
    plugin_root.mkdir(parents=True)
    (plugin_root / "__init__.py").write_text(
        "class DemoPlugin(_PluginBase):\n    pass\n",
        encoding="utf-8",
    )
    event_path = plugin_root / "worker.py"
    ...
    assert resolver.runtime_plugin(event_path) == "DemoPlugin"
