...
from app.runtime.compat.readiness import (
...
def test_multi_version_blockers_aggregates_self_referential_and_shared_base_hits(
    tmp_path: Path,
) -> None:
...
def test_multi_version_blockers_is_empty_for_clean_source_dirs(tmp_path: Path) -> None:
    ...
    second.mkdir()
    (second / "__init__.py").write_text("value = 2\n", encoding="utf-8")
