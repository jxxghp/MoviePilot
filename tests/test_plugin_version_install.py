...
def test_register_plugin_version_writes_manifest_and_sets_current(tmp_path: Path) -> None:
    """注册一个版本后元信息登记该版本并置为当前版本，返回其版本目录名与登记前的当前版本号。"""
    plugin_root = tmp_path / "registered"

    dir_name, previous_current = register_plugin_version(plugin_root, "1.2.0", source="local")
...
def test_register_plugin_version_keeps_other_versions_and_switches_current(
    ...
    """注册第二个版本后两条记录并存，当前版本切到新注册的版本，返回登记前的当前版本号。"""
    plugin_root = tmp_path / "dual"
    register_plugin_version(plugin_root, "1.0.0", source="local")

    _, previous_current = register_plugin_version(plugin_root, "2.0.0", source="local")
...
def test_version_switch_with_self_referential_import_is_rejected(
    monkeypatch: pytest.MonkeyPatch,
    tmp_path: Path,
) -> None:
    ...
    _install_pinned_host_binding(monkeypatch, "1.0.0")
...
def test_version_switch_is_still_checked_under_versioned_layout(
    monkeypatch: pytest.MonkeyPatch,
    tmp_path: Path,
) -> None:
    """插件已迁到版本目录布局后，安装期并存检查必须照常生效。

    迁移会把源码整体搬进版本目录，插件根目录从此不再有 __init__.py；若守卫仍按
    根目录判断已装源码，就会从第二个版本开始每次直接放行，永久失效。
    """
    plugin_dir = tmp_path / "app" / "plugins" / "demoplugin"
    installed_dir = plugin_dir / plugin_version_dir_name("1.0.0")
    installed_dir.mkdir(parents=True)
    (installed_dir / "__init__.py").write_text(
        "from app.plugins.demoplugin.utils import helper\n"
        "class DemoPlugin:\n    plugin_version = '1.0.0'\n",
        encoding="utf-8",
    )
    (installed_dir / "utils.py").write_text("def helper():\n    pass\n", encoding="utf-8")
    source_dir = tmp_path / "repo" / "demoplugin"
    _write_flat_plugin(source_dir, class_name="DemoPlugin", version="2.0.0")
    _install_pinned_host_binding(monkeypatch, "1.0.0")
...
def test_version_switch_with_shared_base_model_is_rejected(
    monkeypatch: pytest.MonkeyPatch,
    tmp_path: Path,
) -> None:
...
def test_single_instance_ordinary_update_is_allowed_without_binding(
    tmp_path: Path,
) -> None:
    """只有一个默认 host 时，磁盘保留旧目录不等于真实版本并存。"""
    plugin_dir = tmp_path / "app" / "plugins" / "demoplugin"
    _write_flat_plugin(plugin_dir, class_name="DemoPlugin", version="1.0.0")
    source_dir = tmp_path / "repo" / "demoplugin"
    _write_flat_plugin(source_dir, class_name="DemoPlugin", version="2.0.0")

    assert reject_incompatible_plugin_version_switch(
        "DemoPlugin", plugin_dir, source_dir
    ) is None


def test_pinned_instances_without_followers_do_not_create_coexistence(
    monkeypatch: pytest.MonkeyPatch,
    tmp_path: Path,
) -> None:
    """全部实例仍钉在旧版时，安装新 current 不构成运行时并存。"""
    plugin_dir = tmp_path / "app" / "plugins" / "demoplugin"
    _write_flat_plugin(plugin_dir, class_name="DemoPlugin", version="1.0.0")
    (plugin_dir / "utils.py").write_text("def helper():\n    pass\n", encoding="utf-8")
    (plugin_dir / "__init__.py").write_text(
    ...
    manager = SimpleNamespace(
        get_plugin_version_binding=lambda _plugin_id: SimpleNamespace(
            pinned_version="1.0.0",
        ),
        get_plugin_source_instances=lambda _plugin_id: [SimpleNamespace(
            pinned_version="1.0.0",
        )],
    )
    monkeypatch.setattr(
...
@pytest.mark.parametrize("missing_host", [True, False])
def test_missing_pinned_directory_counts_current_fallback_in_coexistence(
    monkeypatch: pytest.MonkeyPatch,
    tmp_path: Path,
    missing_host: bool,
) -> None:
    """缺失绑定目录回落新 current 时，仍与另一实例的旧版构成并存。"""
    plugin_dir = tmp_path / "demoplugin"
    _write_flat_plugin(plugin_dir, class_name="DemoPlugin", version="1.0.0")
    ...
    plugin_dir.joinpath("utils.py").write_text("def helper(): pass\n", encoding="utf-8")
    source_dir = tmp_path / "source"
    _write_flat_plugin(source_dir, class_name="DemoPlugin", version="2.0.0")
    host = SimpleNamespace(
        pinned_version="9.0.0" if missing_host else "1.0.0",
    )
    clone = SimpleNamespace(
        pinned_version="1.0.0" if missing_host else "9.0.0",
    )
    monkeypatch.setattr(
        "app.application.plugin.runtime.get_existing_plugin_manager",
        lambda: SimpleNamespace(
            get_plugin_version_binding=lambda _plugin_id: host,
            get_plugin_source_instances=lambda _plugin_id: [clone],
        ),
    )
...
def test_coexistence_guard_scans_pinned_historical_version(
    monkeypatch: pytest.MonkeyPatch,
    tmp_path: Path,
) -> None:
    """current 已是 2.0 时，host 钉 1.0 的并存体检必须扫描 1.0。"""
    plugin_dir = tmp_path / "app" / "plugins" / "demoplugin"
    old_dir = plugin_dir / plugin_version_dir_name("1.0.0")
    current_dir = plugin_dir / plugin_version_dir_name("2.0.0")
    _write_flat_plugin(old_dir, class_name="DemoPlugin", version="1.0.0")
    old_dir.joinpath("__init__.py").write_text(
        "from app.plugins.demoplugin.utils import helper\n"
        "class DemoPlugin:\n    plugin_version = '1.0.0'\n",
        encoding="utf-8",
    )
    old_dir.joinpath("utils.py").write_text("def helper():\n    pass\n", encoding="utf-8")
    _write_flat_plugin(current_dir, class_name="DemoPlugin", version="2.0.0")
    register_plugin_version(plugin_dir, "1.0.0", source="test")
    register_plugin_version(plugin_dir, "2.0.0", source="test")
    source_dir = tmp_path / "repo" / "demoplugin"
...
def _install_pinned_host_binding(
    monkeypatch: pytest.MonkeyPatch,
    version: str,
) -> None:
    """为安装守卫提供一个实际钉住旧版本的 host 绑定。"""
    manager = SimpleNamespace(
        get_plugin_version_binding=lambda _plugin_id: SimpleNamespace(
            pinned_version=version,
        ),
        # 一个跟随 current 的分身使切换后确实同时运行旧 host 与新分身，
        # 守卫此时才应执行多版本写法体检。
        get_plugin_source_instances=lambda _plugin_id: [SimpleNamespace(
            pinned_version=None,
        )],
...
def test_remove_plugin_installed_version_keeps_sibling_versions_and_restores_previous_current(
    ...
    """回滚失败版本只删该版本目录与元信息条目，当前版本精确复原为调用方传入的登记前的值。

    ``previous_current`` 传入的是 1.0.0，既不是剩余版本里语义号最高的
    2.0.0，也不是被删版本 3.0.0 之前链式注册产生的值，用来证明复原结果
    只取决于调用方传入的值本身，不是按剩余版本重新猜一个。
    """
    ...
    remove_plugin_installed_version(plugin_root, "3.0.0", "1.0.0")
...
def test_remove_plugin_installed_version_falls_back_to_empty_when_previous_current_is_gone(
    tmp_path: Path,
) -> None:
    """登记前的当前版本在回滚时已不在剩余版本清单中（理论上不该发生）时，置空而不是乱猜。"""
    plugin_root = tmp_path / "demoplugin"
    version_a = _register_real_version(plugin_root, "1.0.0", marker="A")
    version_c = _register_real_version(plugin_root, "3.0.0", marker="C")

    remove_plugin_installed_version(plugin_root, "3.0.0", "9.9.9")
...
def test_remove_plugin_installed_version_is_a_no_op_when_the_version_was_never_placed(
    ...
    remove_plugin_installed_version(plugin_root, "9.9.9", "1.0.0")
...
def test_sync_install_failure_restores_current_to_the_version_installed_before_this_attempt(
    monkeypatch: pytest.MonkeyPatch, tmp_path: Path,
) -> None:
    """安装前 current 不是剩余版本里语义号最高者时，回滚同样精确复原为安装前的真实值，不回退到最高版本。

    版本 B（2.0.0）先于版本 A（1.0.0）注册，注册顺序决定安装第三个版本前
    ...
    version_b = _register_real_version(existing_dir, "2.0.0", marker="B")
    version_a = _register_real_version(existing_dir, "1.0.0", marker="A")
    assert read_plugin_versions_manifest(existing_dir)["current"] == "1.0.0"
...
@pytest.mark.asyncio
async def test_async_install_failure_restores_current_that_is_neither_the_highest_nor_the_lowest_version(
    monkeypatch: pytest.MonkeyPatch, tmp_path: Path,
) -> None:
    """安装前 current 是三个已装版本里居中的一个时，回滚同样精确复原，既不落到最高也不落到最低。"""
    manager, plugin_root = _versioned_manager(monkeypatch, tmp_path)
    existing_dir = plugin_root / "demoplugin"
    version_a = _register_real_version(existing_dir, "1.0.0", marker="A")
    version_d = _register_real_version(existing_dir, "4.0.0", marker="D")
    version_b = _register_real_version(existing_dir, "2.0.0", marker="B")
    assert read_plugin_versions_manifest(existing_dir)["current"] == "2.0.0"

    async def prepare(staging_dir: Path) -> tuple[bool, str]:
        """准备一份声明第四个版本号的替换内容。"""
        _write_flat_plugin(staging_dir, class_name="DemoPlugin", version="5.0.0")
        return True, ""
    ...
    assert success is False
    assert message == "依赖安装失败：模拟"
    assert not (existing_dir / "v5_0_0").exists()
    assert (version_a / "marker.py").read_text(encoding="utf-8") == "A"
    assert (version_b / "marker.py").read_text(encoding="utf-8") == "B"
    assert (version_d / "marker.py").read_text(encoding="utf-8") == "D"
    manifest = read_plugin_versions_manifest(existing_dir)
    assert {entry["version"] for entry in manifest["versions"]} == {"1.0.0", "2.0.0", "4.0.0"}
    assert manifest["current"] == "2.0.0"


def test_sync_install_failure_for_the_first_ever_version_leaves_no_empty_shell(
    monkeypatch: pytest.MonkeyPatch, tmp_path: Path,
) -> None:
    """插件在安装这一次之前没有任何已装版本、首个版本安装失败时，回滚后不留没有可用版本的空壳目录。"""
    manager, plugin_root = _versioned_manager(monkeypatch, tmp_path)
    existing_dir = plugin_root / "demoplugin"
