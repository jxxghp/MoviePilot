"""插件源码按版本分目录布局的目录名映射、元信息读写、加载路径解析与路径识别测试。"""
...
def _make_loader(plugins_root: Path, **kwargs) -> PluginLoader:
    """构造只需最小日志端口的加载器实例。

    :param plugins_root: 插件根目录
    :param kwargs: 透传给加载器的可选端口，如 host_binding
    :return: 加载器实例
    """
    return PluginLoader(
...
def _write_flat_plugin(plugins_root: Path, plugin_id: str, *, class_name: str) -> Path:
    """在插件根目录下直接写入平铺布局的最小可加载源码。
    ...
    (plugin_root / "__init__.py").write_text(
        f"class {class_name}(_PluginBase):\n"
...
def _make_path_resolver(runtime_root: Path, running: dict) -> PluginPathResolver:
    """构造只依赖临时运行目录、不触碰真实插件市场的路径解析器。

    :param runtime_root: 插件运行目录
    :param running: 已运行插件 ID 到插件实例的映射
    :return: 路径解析器
    """
    system = SimpleNamespace(
        local_repo_paths=lambda: [],
        local_candidate=lambda *_args, **_kwargs: None,
    )
    return PluginPathResolver(
        runtime_root=runtime_root,
        running=lambda: running,
        system=lambda: system,
        strict_system_version=lambda: True,
        log=_logger(),
    )
...
def test_resolve_falls_back_to_the_plugin_root_without_any_version_directory(
    ...
    """
    plugin_root = _write_flat_plugin(tmp_path, "flat", class_name="FlatPlugin")
...
def test_loader_uses_the_host_binding_version_when_one_is_bound(
    tmp_path: Path,
) -> None:
    """插件本体被绑定到某个版本时按该版本加载，而不是元信息的当前版本。"""
    _write_version(tmp_path, "bound", "1.0.0", class_name="BoundPlugin")
    _write_version(tmp_path, "bound", "2.0.0", class_name="BoundPlugin")
    plugin_root = tmp_path / "bound"
    _write_manifest(
        plugin_root, [("1.0.0", "v1_0_0"), ("2.0.0", "v2_0_0")], current="2.0.0"
    )

    loader = _make_loader(tmp_path, host_binding=lambda _plugin_id: "1.0.0")
    plugins = loader.load(
        None, ["Bound"], lambda candidate: hasattr(candidate, "init_plugin")
    )

    assert plugins[0].plugin_version == "1.0.0"


def test_loader_falls_back_to_the_current_version_when_the_binding_is_stale(
    tmp_path: Path,
) -> None:
    """绑定的版本目录已不在磁盘上时回落到当前版本，而不是让本体加载失败。"""
    _write_version(tmp_path, "gone", "2.0.0", class_name="GonePlugin")
    plugin_root = tmp_path / "gone"
    _write_manifest(plugin_root, [("2.0.0", "v2_0_0")], current="2.0.0")

    loader = _make_loader(tmp_path, host_binding=lambda _plugin_id: "1.0.0")
    plugins = loader.load(
        None, ["Gone"], lambda candidate: hasattr(candidate, "init_plugin")
    )
...
def test_flat_layout_loading_path_is_identical_to_before_versioned_layout(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    """没有版本目录时，加载路径与解析结果与引入版本化布局之前完全一致。

    这是本层唯一的验收判据：存量插件既不会被改走按版本目录手动构造模块规格的
    分支，导入前置能力拿到的也仍然是插件根目录本身，而不是任何解析出来的
    新目录，因此插件侧可观测的行为为零变化。
    """
    plugins_package = importlib.import_module("app.plugins")
    monkeypatch.setattr(
    ...
    plugin_root = tmp_path / "unchanged"
    plugin_root.mkdir(parents=True)
    (plugin_root / "__init__.py").write_text(
        "class UnchangedPlugin:\n"
        "    plugin_version = '1.0.0'\n"
        "    def init_plugin(self, config=None):\n"
        "        pass\n",
        encoding="utf-8",
    ...
    def _reject_versioned_import(*_args, **_kwargs):
        """版本化导入分支被走到即判定行为已改变。"""
        raise AssertionError("平铺布局不应走按版本目录构造模块规格的导入分支")

    monkeypatch.setattr(
        PluginLoader, "_import_versioned_module", staticmethod(_reject_versioned_import)
    )
    loader = PluginLoader(
        plugins_root=tmp_path,
        import_preparer=lambda *, plugin_id, plugin_dir: prepared.append(plugin_dir),
        import_scanner=lambda *, plugin_id, plugin_dir: scanned.append(plugin_dir),
        log=_logger(),
    )

    plugins = loader.load(
        None, ["Unchanged"], lambda candidate: hasattr(candidate, "init_plugin")
    )

    assert [plugin.__name__ for plugin in plugins] == ["UnchangedPlugin"]
    assert prepared == [plugin_root]
    assert scanned == [plugin_root]
    assert sys.modules["app.plugins.unchanged"].__file__ == str(
...
def test_runtime_plugin_resolves_a_class_inside_a_version_directory(
    tmp_path: Path,
) -> None:
    """事件落在版本目录内时按该版本目录的主模块解析插件类名。"""
    version_dir = tmp_path / "monitored" / "v1_2_0"
    version_dir.mkdir(parents=True)
    (version_dir / "__init__.py").write_text(
        "class MonitoredPlugin(_PluginBase):\n    pass\n", encoding="utf-8"
    )
    resolver = _make_path_resolver(tmp_path, running={})

    assert resolver.runtime_plugin(version_dir / "utils.py") == "MonitoredPlugin"


def test_runtime_plugin_still_resolves_a_flat_layout_plugin(tmp_path: Path) -> None:
    """平铺布局下仍按插件根目录的主模块解析，识别结果不受版本目录支持影响。"""
    plugin_root = _write_flat_plugin(tmp_path, "flatmonitored", class_name="FlatMonitored")
    resolver = _make_path_resolver(tmp_path, running={})

    assert resolver.runtime_plugin(plugin_root / "utils.py") == "FlatMonitored"


def test_runtime_plugin_ignores_a_non_version_subdirectory(tmp_path: Path) -> None:
    """子目录名不是版本目录时不下探，插件根目录没有主模块即丢弃事件。"""
    nested = tmp_path / "nested" / "dist"
    nested.mkdir(parents=True)
    (nested / "__init__.py").write_text(
        "class NestedPlugin(_PluginBase):\n    pass\n", encoding="utf-8"
    )
    resolver = _make_path_resolver(tmp_path, running={})

    assert resolver.runtime_plugin(nested / "remoteEntry.js") is None


def test_federated_change_is_scoped_to_the_current_version_directory(
    tmp_path: Path,
) -> None:
    """联邦产物遏制范围收到当前版本目录：当前版本内的构建事件被识别并判定就绪。"""
    version_dir = _write_version(tmp_path, "fed", "2.0.0", class_name="FedPlugin")
    _write_version(tmp_path, "fed", "1.0.0", class_name="FedPlugin")
    _write_manifest(
        tmp_path / "fed", [("1.0.0", "v1_0_0"), ("2.0.0", "v2_0_0")], current="2.0.0"
    )
    dist_dir = version_dir / "dist"
    dist_dir.mkdir()
    (dist_dir / "remoteEntry.js").write_text("//\n", encoding="utf-8")
    running = {"fed": SimpleNamespace(get_render_mode=lambda: ("vue", "dist"))}
    resolver = _make_path_resolver(tmp_path, running=running)

    assert resolver.federated_change(dist_dir / "remoteEntry.js") == ("fed", None, True)
    assert (
        resolver.federated_change(tmp_path / "fed" / "v1_0_0" / "dist" / "remoteEntry.js")
        is None
    )


def test_federated_change_is_unchanged_for_a_flat_layout_plugin(tmp_path: Path) -> None:
    """平铺布局下联邦产物仍以插件根目录为遏制范围，判定结果与引入前一致。"""
    plugin_root = _write_flat_plugin(tmp_path, "flatfed", class_name="FlatFedPlugin")
    dist_dir = plugin_root / "dist"
    dist_dir.mkdir()
    (dist_dir / "remoteEntry.js").write_text("//\n", encoding="utf-8")
    running = {"flatfed": SimpleNamespace(get_render_mode=lambda: ("vue", "dist"))}
    resolver = _make_path_resolver(tmp_path, running=running)

    assert resolver.federated_change(dist_dir / "remoteEntry.js") == (
        "flatfed",
        None,
        True,
    )
