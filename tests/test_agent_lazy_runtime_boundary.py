...
def test_legacy_orchestrator_tool_factory_symbol_keeps_canonical_identity() -> None:
    """旧 monkeypatch 路径应解析到 Runtime 持有的真实工厂类。"""
    from app.agent import orchestrator
    from app.agent.runtime_loader import get_tool_factory
    from app.agent.tools import manager as tools_manager

    factory = get_tool_factory()
    assert orchestrator.MoviePilotToolFactory is factory
    assert tools_manager.MoviePilotToolFactory is factory

    with patch.object(factory, "catalog_factory_revision", return_value="patched"):
        assert orchestrator.MoviePilotToolFactory.catalog_factory_revision() == "patched"
        assert tools_manager.MoviePilotToolFactory.catalog_factory_revision() == "patched"


def test_orchestrator_star_import_preserves_lazy_canonical_factory() -> None:
    """普通导入保持冷态，显式与星号导入按需返回 canonical 工厂。"""
    result = _run_isolated(
        """
import json
import sys
...
cold = {
    "module_loaded": "app.agent.tools.factory" in sys.modules,
    "materialized": is_tool_factory_materialized(),
}
expected_exports = {
    name
...
explicit_namespace = {}
exec(
    "from app.agent.orchestrator import MoviePilotToolFactory as ExplicitFactory",
    explicit_namespace,
)
star_namespace = {}
exec("from app.agent.orchestrator import *", star_namespace)

from app.agent.runtime_loader import get_tool_factory
canonical = get_tool_factory()
print(json.dumps({
    "cold": cold,
    "all_exact": set(orchestrator.__all__) == expected_exports,
    "explicit_identity": explicit_namespace["ExplicitFactory"] is canonical,
    "star_identity": star_namespace["MoviePilotToolFactory"] is canonical,
    "star_has_agent": star_namespace["MoviePilotAgent"] is orchestrator.MoviePilotAgent,
}))
"""
    )

    assert result == {
        "cold": {"module_loaded": False, "materialized": False},
        "all_exact": True,
        "explicit_identity": True,
        "star_identity": True,
        "star_has_agent": True,
