...
def _install_version(plugin_root: Path, version: str) -> Path:
    ...
    (version_dir / "__init__.py").write_text(f"plugin_version = {version!r}\n", encoding="utf-8")
...
def test_current_version_is_never_recycled(tmp_path: Path) -> None:
    ...
    outcome = recycle_plugin_version_directories(plugin_root, referenced_versions=set(), retention=0)
...
def test_pending_installation_skips_recycle_to_preserve_commit_receipt(
    tmp_path: Path,
) -> None:
    """未收尾安装 journal 存在时不改插件根目录，保持 COMMITTED 收据可验证。"""
    plugin_root = tmp_path / "sample"
    _install_version(plugin_root, "1.0.0")
    _install_version(plugin_root, "2.0.0")
    before = {
        path.name: path.read_bytes()
        for path in (plugin_root / "v1_0_0", plugin_root / "v2_0_0")
    ...
    assert outcome["removed"] == []
    assert all("未收尾安装事务" in reason for reason in outcome["kept"].values())
    after = {
        path.name: path.read_bytes()
        for path in (plugin_root / "v1_0_0", plugin_root / "v2_0_0")
        for path in path.rglob("*")
...
def test_retention_window_keeps_the_n_most_recent_versions(tmp_path: Path) -> None:
    ...
    outcome = recycle_plugin_version_directories(plugin_root, referenced_versions=set(), retention=2)
...
def test_missing_installed_at_is_treated_as_oldest(tmp_path: Path) -> None:
    ...
    outcome = recycle_plugin_version_directories(plugin_root, referenced_versions=set(), retention=1)
...
def test_manifest_is_updated_after_recycling(tmp_path: Path) -> None:
    ...
    recycle_plugin_version_directories(plugin_root, referenced_versions=set(), retention=0)
...
def test_recycle_window_keeps_newest_when_installed_at_is_missing(tmp_path: Path) -> None:
    """登记时间全部缺失时，保留窗口要保住最新的版本而不是最旧的。

    清单丢失或损坏会让每个版本的登记时间都读成空串、排序键全相等；稳定排序不
    反转等值元素，窗口会退化成保留目录名字典序最靠前的那几个（也就是最旧的
    几个），把当前正在加载的最新版本删掉。
    """
    plugin_root = tmp_path / "sample"
