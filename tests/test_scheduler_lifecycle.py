...
def anyio_backend() -> str:
    """限定异步用例使用调度器句柄所有权依赖的 asyncio 后端。

    本模块断言的正是 ``asyncio.create_task`` / ``run_coroutine_threadsafe`` 的
    句柄归属与取消收口，在 trio 后端下没有 running asyncio loop，必然以
...
async def test_stale_progress_cannot_update_replaced_job(monkeypatch) -> None:
    ...
    callback = scheduler._SchedulerEngine__build_progress_callback(
        "generation-progress",
        old_job,
    )
...
async def test_replaced_job_keeps_active_state_without_stale_progress(monkeypatch) -> None:
    ...
    old_job = scheduler._SchedulerEngine__prepare_job("generation-cache")
...
async def test_stale_generation_cannot_finish_replaced_job(monkeypatch) -> None:
    ...
    await scheduler._SchedulerEngine__finish_job(
...
def test_stop_between_prepare_and_submission_releases_active_generation(
    ...
    original_prepare = scheduler._SchedulerEngine__prepare_job
    ...
    monkeypatch.setattr(scheduler, "_SchedulerEngine__prepare_job", prepare_then_stop)
    ...
    monkeypatch.setattr(scheduler, "_SchedulerEngine__prepare_job", original_prepare)
    scheduler._lifecycle_state = "running"
...
async def test_cross_thread_rejection_closes_unstarted_business_coroutine(
    ...
    monkeypatch.setattr(scheduler, "_SchedulerEngine__prepare_job", prepare_then_wait)
