...
def test_manual_subscription_search_uses_task_registry(monkeypatch) -> None:
    ...
    # 一个请求的全部搜索目标合并为同一个批次任务，登记的是批次入口而不是调度器本身。
    ...
    monkeypatch.setattr(
        ...
        "start_scheduler_job",
        lambda *call_args, **call_kwargs: scheduled.append((call_args, call_kwargs)),
    )
