...
def _patch_lifespan(monkeypatch, *, failing_step: str | None = None) -> dict:
    ...
    monkeypatch.setattr(lifecycle, "settle_events", shutdown_steps["settle_events"])
    monkeypatch.setattr(
        lifecycle,
...
def test_lifecycle_manifest_declares_normal_and_safe_mode_order() -> None:
    ...
    # 宿主生产任务（插件监控、定时器、Agent、整理）先停；随后封口插件变更、停用插件
    # 事件入口并结算在途 handler；最后才调用插件私有 close/stop_service 旧停机 hook，
    # 由事件投递屏障封住新投递后卸载插件实例。
    assert normal_stop == [
        "后台任务登记器",
        "插件变更监控",
    ...
    assert all(
        ...
        for item in normal
        if item["name"] not in fail_fast_stop_names
    )
    assert all(
