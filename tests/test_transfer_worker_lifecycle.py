...
from app.startup import transfer_initializer as transfer_initializer
...
def test_failed_retry_schedule_registers_future_observer(monkeypatch) -> None:
    ...
    # global_vars.loop 会先核对登记循环仍在运行且未关闭，替身必须给出确定的布尔值，
    # 否则 MagicMock 的默认返回值会让 is_closed() 恒真而拒绝提交。
