...
from app.api.endpoints import pluginloglevel as pluginloglevel_endpoint
from app.api.endpoints.pluginloglevel import (
...
def test_get_returns_manager_levels_wrapped_in_plugin_overview(monkeypatch):
    ...
    插件实例数量始终很小，因而包一层 plugin_id + instances。
    """
    levels = [
        {
    ...
    monkeypatch.setattr(pluginloglevel_endpoint, "get_plugin_manager", lambda: manager)
