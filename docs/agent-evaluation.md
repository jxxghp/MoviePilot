...
前端后续修复把未收到稳定 `steering_message_id` 的本地草稿改为有序集合，并在 queued ACK 阶段直接插入当前助手段之后的真实边界；queued ACK 同时封闭当前助手段、创建 continuation，使 ACK 之后到达的文本和工具事件不会越过用户消息，applied 事件再绑定稳定 ID、更新状态，若边界尚未建立则补做切分。这样主流事件、短 ACK、断流恢复或多个补充消息交错时，用户消息从首次显示开始就保持工具调用之间的顺序，不会先落在列表末尾再跳到顶部，也不会因单一草稿引用被覆盖。新增回归覆盖两个 ACK 断流后按提交顺序应用的补充消息，以及 queued 插入后仍按顺序承接后续工具；前端提交 `e39d3f0b928de2e1cf4ef935eeb0614f9441917d`，`AgentAssistantPanel.spec.ts` 现为 44 项，类型检查、格式和 ESLint 均通过，远端 `Frontend Tests` run [34618601646](https://github.com/jxxghp/MoviePilot-Frontend/actions/runs/34618601646) 成功。
