...
生产者与消费者共用 `scripts/architecture/event_facts.py` 这一份逐调用事实源。当前宿主有 93 个
生产调用，其中 92 个静态解析为 94 个事件引用，只有 `Command.send_plugin_event` 的插件事件类型
...
- `event_facts` 是生产者/消费者唯一收集源；运行快照记录 93 个生产调用和 17 个消费注册，
...
| Event Contract | 53 |
| Event producer / consumer | 93（92 静态、1 动态）/ 17（16 静态、1 动态） |
