...
`plugin.versions.get` 使用源插件 ID 查询已安装版本以及本体、分身的绑定。实例项的
`follow_current_version` 和 `plugin_version` 描述持久化绑定；`running_version` 是当前
运行实例实际加载的版本，实例停止时为 `null`，不得用绑定版本代替运行结果。

`plugin.versions.set_instance` 使用该总览返回的精确 `instance_id`，切换成功后刷新该
实例的宿主注册信息。切换失败会尝试恢复原绑定及原运行版本；应读取响应的 `success`
和 `message`，并重新查询版本总览、运行状态和插件列表，不得把 HTTP 200 当作切换成功。
切回“跟随当前版本”同样接受多版本并存兼容性检查。

`plugin.versions.recycle` 在插件包写入或安装事务尚未收尾时保留版本目录，避免改变
恢复校验所需的载荷收据；应查看返回的 `removed` 与 `kept`，不能把成功响应理解为
一定删除了旧版本。

插件联邦发现响应及认证提供方的 `remote` 描述保留实例 `id`，并增加实际运行的
`version` 与按版本区分的 `remote_key`。客户端使用 `id` 调用实例 API，使用
`remote_key` 注册联邦组件，并原样使用返回的 `url`；版本目录写入 URL 路径，使
相对引用的 JavaScript、CSS 等资源也按版本隔离。切换后需要重新发现并注册远程
入口，不能继续使用旧实例 ID 对应的组件缓存。

源码版本保留不等于配置或插件自有数据库快照。插件版本共用进程中的第三方依赖，
同一实例切换版本继续使用原数据；旧版不认识当前数据库迁移版本时，不能通过强制
修改绑定来保证回退，应按插件的数据兼容策略处理。
