...
| 下载事实 | `note`、`downloaded_tracks`、`episode_priority` | 主程序或可信回填入口确认已经存在或已经下载的目标。`note` 表示影视/单曲事实，`downloaded_tracks` 保存专辑已接受资源中的去重音轨键，`episode_priority` 表示电视剧每集已知下载质量。 | 下载事实入口、backfill 入口、reset 入口 |
| 进度摘要 | `lack_episode`、`current_priority`、`completed_episode`、`completed_tracks` | 面向搜索、完成判定和展示的进度。`current_priority` 是当前洗版模式的资源准入基线；`completed_episode` 和 `completed_tracks` 由响应层按当前事实计算。 | progress 刷新入口、下载事实入口、响应构造 |
| 生命周期状态 | `state`、`last_update` | 订阅是否继续搜索，以及最近一次系统写入时间。 | 搜索下载流程、完成订阅、reset、状态更新 |

字段所有权应保持集中：下载事实入口写事实，progress 刷新入口写派生进度，完成入口写完成状态，普通更新入口不应绕过这些语义直接改派生字段。
...
| 下载选择 | 下载链路 | 将候选资源交给下载器；专辑下载层从文件清单提取可识别音轨键，不再按当前种子文件数拦截。 | 下载上下文、剩余缺口 |
| 事实记录 | 下载任务添加成功或可信 backfill | 写入 `note`，专辑合并 `downloaded_tracks`，电视剧同步写入或提升 `episode_priority`；全集洗版仅在下载层确认完整覆盖后提升 `current_priority`。 | 订阅下载事实快照 |
| 进度刷新 | 下载流程、backfill、剧集范围变化、显式刷新 | 普通电视剧按媒体库缺失结果刷新 `lack_episode`；电视剧洗版按目标范围和按集事实刷新 `lack_episode`，仅分集洗版从按集事实派生 `current_priority`。 | 进度字段 |
...
| 全集洗版 | 电视剧 | 完整目标集范围的整包质量 | 整包下载确认完整覆盖后，按目标范围写 `note` 和 `episode_priority`，并提升全集准入基线 `current_priority`。 | `current_priority` 表示全集模式的资源准入基线；保留的按集事实不参与候选过滤。 | `current_priority == 100` 或完成策略满足时完成。 | 用完整季包替换已有剧集 |
| 普通专辑订阅 | 音乐专辑 | 专辑总曲目范围 | 每次成功添加资源后合并去重的 `downloaded_tracks`。 | `completed_tracks` 为已累计曲目数，不能超过 `total_tracks`。 | 累计曲目数达到 `total_tracks` 后完成；单个不完整资源也可继续累计。 | 分批收集不同资源中的专辑曲目 |
...
- `current_priority` 对分集洗版是目标范围内最低已知质量摘要，缺失质量事实按 `0` 参与计算。

普通专辑订阅使用独立音轨事实：下载层从种子文件清单提取盘号、曲序或归一化曲名组成的稳定键，订阅事实入口跨轮次合并并去重。接口只返回派生的 `completed_tracks`，不暴露内部音轨键；达到 `total_tracks` 前保持订阅，达到后才进入完成流程。

因此普通电视剧订阅和分集洗版可以互相转换。普通订阅下载产生的按集质量事实可被分集洗版继续使用；分集洗版产生的 `note` 也可被普通订阅继续作为已下载事实。

全集洗版关注完整目标范围的整体资源质量。整包候选必须完整覆盖 `[start_episode, total_episode]`，不是只覆盖当前缺口集。下载层确认完整覆盖后，主程序才可提升全集 `current_priority`，并在资源缺少显式集数时按目标范围补写按集事实。`episode_priority` 在全集模式中继续只升不降，供以后切回分集洗版使用，但不参与全集候选过滤和完成判断。

全集目标范围扩大后，原整包不再能证明覆盖新范围，因此全集准入基线 `current_priority` 重置为 `0`；目标范围缩小时，原整包仍覆盖新范围，保留该基线。
...
| `downloaded_tracks` | 专辑已接受资源中的去重音轨稳定键。 | 只存在于活动订阅内部事实；API 通过 `completed_tracks` 暴露数量。 |
...
| `current_priority` | 当前洗版模式的资源准入基线。 | 电影和全集洗版由对应模式的受控入口维护；分集洗版按目标范围内最低已知质量刷新。 |
| `completed_episode` | 展示用完成集数。 | 分集洗版按顶档剧集数计算，全集洗版按整包准入基线是否达到顶档计算；不作为主要事实源。 |
| `completed_tracks` | 展示用已完成专辑曲目数。 | 由 `downloaded_tracks` 去重计数并截断到 `total_tracks`；不作为写入字段。 |
...
| 想用完整季包替换整季 | 全集洗版 | 优先寻找完整覆盖目标范围的整包资源，并按全集准入基线判断是否下载。 | 整包必须覆盖目标集范围，不只是覆盖当前缺口。 |
...
`last_search` 记录完整搜索轮次的开始时间，供订阅独立周期或系统周期判断下一次搜索是否到期。
因站点暂时不可用而仅补查剩余站点时，不刷新此时间，避免坏站点持续推迟正常站点的新一轮搜索。
同一轮恢复保留待搜索站点；经过到期筛选的新周期会把尚未运行的恢复任务重新覆盖当前启用站点。
正在执行的任务保持原范围，由后续周期调度接续，不改变已开始的下载副作用。
