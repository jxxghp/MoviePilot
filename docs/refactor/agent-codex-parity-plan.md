...
- `terminal_session` 在同一 `gpt-5.6-luna + max`、OAuth 和修复后 Harness 下的配对 `/tmp/moviepilot-agent-round-luna-oauth-terminal-appserver-pair-final-d22fa746f.json` 为 `pair_valid=true`、`both_passed=false`。MoviePilot 报告 `/tmp/moviepilot-agent-round-luna-oauth-live-terminal-appserver-final-d22fa746f.json` 4 次模型调用完成 `start → write(session_id) → read`，真实 pipe 输出与退出码通过；原生 app-server 报告 `/tmp/moviepilot-agent-round-luna-oauth-native-terminal-appserver-final-d22fa746f.json` 仍观察到 stdin 在 READY 前关闭，评分器保留 `terminal_input_not_verified`、`terminal_output_not_read` 和 `terminal_output_not_verified`。这证明一次性 pipe 执行器的边界仍然存在，不能把工具清单广告视为交互能力通过。
- 对应的 `terminal_pty_session` 原生报告 `/tmp/moviepilot-agent-round-luna-oauth-native-terminal-pty-appserver-final-d22fa746f.json` 已通过 app-server 的结构化事件记录到命令完成、`terminalInteraction` 和输出增量，但本次模型流在最终 JSON 前因 SSE idle timeout 断开，独立验收为 `invalid_final_report`。当前改动提升了证据采集能力，没有把未核验的 PTY 终态改判为通过；`harness_sha256` 为 `aac8905480b95343c17875ed240f9f6b8ca9ea65bc97647f2696a1deb0a64497`。
