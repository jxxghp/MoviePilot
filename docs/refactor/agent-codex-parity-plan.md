...
| S2.3 运行中消息排队与 WebAgent 输入 | 实现完成，三轮真实配对通过 | 运行中仍可提交新消息；消息按会话原子入队，在下一次模型调用边界注入真实 `HumanMessage`；SSE 报告 queued/applied，稳定 `steering_message_id` 贯穿展示快照、前端本地状态和恢复合并；应用点收口当前助手并创建 continuation 助手，用户气泡落在真实事件边界；文本、工具、主动消息、错误和终态事件携带 `assistant_message_id`，前端按事件身份回放迟到事件；工具按 `tool_id` 报告 running/done/error；停止后不再派发后续工具 |
...
| S3.1 通用子代理 | 并行与取消恢复均有真实配对通过，终端分享收益仍待验证 | 主 Agent 只暴露并派发 `general-purpose`；旧的专用画像已删除，不保留兼容入口。`subagent_parallel_status` 三轮配对验证两个并行只读子任务、授权、工具角色和零副作用；新增 `subagent_cancel_recovery` 配对验证派发后真实取消、主任务继续读取站点和零副作用。取消时序可能让一侧在子代理模型请求前收口，也可能中断请求，因此该轮只比较行为，token/成本差保持 unknown；终端分享收益仍待证据 |
...
4. **S3.1-R1 终端协作收益**：取消恢复场景已经形成一轮真实行为配对，继续补充通用子代理共享终端和任务封口场景；主动取消造成的未完整用量只能保留行为结论，不能估算 token 成本。没有收益证据就维持单一通用子代理体系。
...
- 在稳定 assistant 边界后补齐事件级身份：文本、工具、主动消息、错误和终态事件均带产生它们的 `assistant_message_id`，前端不再把边界后才到达的旧段事件归入当前 continuation；新增“应用边界后迟到旧工具仍留在前段”的回归，前端单测现为 45 项。后端与前端事件身份修复待推送并等待各自最终 SHA CI。
