...
`tool.uv.conflicts` 保证两个组不能同时解析。`app.runtime.dependencies.runtime_dependency_group()`
根据当前解释器的 `Py_GIL_DISABLED` 构建标志选择 profile；Docker 初始构建、源码升级、启动恢复、
插件安装后的宿主恢复和一致性检查都必须复用这个选择结果。
...
| `crcmod` / `crcmod-plus` | `crcmod` 1.7 | `crcmod-plus` 2.3.1 | `oss2` 元数据仍声明 `crcmod`，V3t 通过 uv 排除该传递依赖并提供兼容模块。只有上游正式接受替代包，或 `crcmod` 自身提供安全的 free-threaded 制品后，才能删除排除规则。 |
...
3. 恢复完成后重新执行项目一致性与核心导入检查；
4. 宿主即使恢复成功，本次插件安装仍返回失败，不能把被回滚的安装报告为成功。

项目一致性检查使用
`uv sync --check --locked --offline --inexact`，它只检查当前环境，不下载或安装依赖。裸
`uv pip check` 仍可作为诊断信息，但不能作为唯一健康结论：V3t 对 `crcmod-plus` 的有意替代会被
`oss2` 的旧元数据报告为缺少 `crcmod`。
...
| 指标 | 标准 V3 | V3t | 结论 |
| --- | ---: | ---: | --- |
| arm64 镜像体积 | 673.1 MB | 702.3 MB | V3t 增加 29.2 MB，约 4.3%；标准镜像仍处于既有 660+ MB 发布口径。 |
| 启动就绪中位数 | 8.418 s | 7.879 s | V3t 约快 6.4%，未造成启动退化；不据此承诺所有部署都会更快。 |
| 空载 working set | 312.5 MiB | 343.9 MiB | V3t 增加约 10.0%，低于 25% 验收阈值。 |
| 主工作负载 PSS | 293.8 MiB | 338.6 MiB | V3t 增加约 15.3%，属于实验镜像的明确资源成本。 |
| 32 线程纯 Python CPU 探针 | 12,160.3 ops/s | 36,245.1 ops/s | V3t 约为 2.98 倍，证明解释器并行收益；该探针不经过 Rust 开关。 |
| 32 线程直接 Rust ABI 探针 | 35,127.9 ops/s | 50,011.9 ops/s | V3t 约为 1.42 倍；两边都直接调用 Rust，仅验证原生 ABI 与并发。 |
| 普通 API 最大 p95 比例 | 基线 | 1.17x | 四个本地 API 中最大退化来自订阅列表，仍低于 1.25x 门禁。 |

该候选还通过了四分片全量回归（5,759 passed、3 skipped、14 subtests passed）、PostgreSQL 18
...
当前六个交替样本的中位耗时分别为 `0.072635s`、`0.038218s`、`0.040049s`，三组业务结果校验和
一致。标准 V3 启用 Rust 后耗时约为 Python fallback 的 `0.526x`；V3t 固定 Rust 路径约为标准 V3
Rust 路径的 `1.048x`，即慢约 4.8%，仍低于 1.25x 门禁。解释器探针的并发收益不能抵销产品路径
退化，因此两组比例必须继续独立验收。

### 6.3 PostgreSQL 三方案

三组使用同一 PostgreSQL 18、相同 SQL 和正反交替顺序，以标准 V3 + `psycopg2` 为 1.00 倍基线：

| 方案 | 单连接查询吞吐 | 16 线程查询吞吐 | 批量事务写耗时 |
| --- | ---: | ---: | ---: |
| V3 + `psycopg2` | 1.00x | 1.00x | 1.00x |
| V3 + `psycopg3 binary` | 0.79x | 0.55x | 0.084x |
| V3t + `psycopg3 C` | 1.70x | 0.87x | 0.089x |

因此标准 V3 保留 `psycopg2`，V3t 为满足 free-threaded ABI 使用 `psycopg3 C`。`psycopg3` 的批量
事务写明显更快，但查询吞吐没有形成全面优势；当前不为统一驱动扩大数据库重构。三种制品内嵌的
libpq 分别为 17.9、18.0 和 18.6，性能差异不能全部归因于 Python 驱动。未来驱动或应用访问模式变化
时，必须重跑真实短事务、批量写、连接池、长事务和迁移场景，不能只用单项微基准改写结论。

### 6.4 环境检查成本

当前 profile 的
`uv sync --check --locked --offline --inexact` 在两套已构建镜像中约为 58-66 ms。它只做离线
一致性检查，适合作为插件安装前后的健康门禁；该数字不是实际依赖恢复耗时。真正恢复可能访问包索引，
耗时仍受缺失制品、缓存命中、网络和原生构建影响，不能用检查耗时推断用户故障恢复时长。
...
空白配置卷首次启动需要下载约 198 MB CloakBrowser 内核，V3t 后端约 6 秒就绪，但容器总启动耗时约
105 秒。该场景接近用户报告的“两分钟启动”，主要瓶颈是持久浏览器缓存未命中和下载网络，不是插件
初始化；缓存命中后的普通重启不重复下载。

在同一 V3t 候选中安装官方 V3 现代清单插件 AgentResourceOfficer 耗时约 17 秒，安装历史
`requirements.txt` 插件 BugReporter 耗时约 3.9 秒，两者均进入 `active` 且没有启用 GIL。删除两份
插件源码并卸载各自顶层依赖后模拟镜像重建，98 个插件快照恢复耗时约 179 ms，Web 在 3 秒先就绪；
两个插件被分类为等待依赖，依赖从持久 uv 缓存后台恢复，并在约 5.9 秒后各加载一次。该结果证明插件
源码恢复不应被依赖下载阻塞，依赖未就绪的插件可以在 Web 可用后渐进加载。

删除一个主程序核心依赖后重启，启动前探针正确触发锁定自愈：uv 解析 208 个包耗时约 26.3 秒，只安装
缺失包约 0.7 秒，总启动约 35 秒。`--inexact` 保留了两个插件的额外依赖，恢复后两者仍为单实例
`active`，V3t GIL 仍关闭。该路径的价值是让受污染共享环境能够恢复启动，不是性能优化；它只在核心
导入失败时执行，正常重启不会承担这段解析成本。
