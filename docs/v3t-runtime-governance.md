...
正式发布为两个变体分别生成同一版本号和 `latest` 标签；两个发布步骤都必须在各自构建、扫描通过后
执行。版本号标签保留用于回溯和固定部署，`latest` 直接由对应的多架构发布步骤写入，不再额外运行
标签晋升步骤。两套镜像仍必须使用同一源码 revision，避免同一发布批次出现来源不一致。
...
插件在 package 索引中用 `v3t: false` 声明不支持 free-threaded 运行时，缺省视为兼容，不要求生态
全量标注。V3t 下该声明只改变可安装性和提示，不改变可见性：

- 市场目录保留条目并标注 `runtime_compatible`、`runtime_message`，前端据此禁用安装并说明原因。
  过滤掉条目会让插件在 V3t 上凭空消失，用户既看不到插件也看不到原因。
- 安装准入统一经 `check_plugin_install_compatibility()` 判定，运行时判据先于宿主版本判据返回，
  在线安装与本地插件包共用这一条。前端禁用只是体验层，服务端必须独立拒绝。
- 运行目录加载仍然跳过声明不兼容的载荷，并把插件状态写为 `incompatible_runtime`，
  由插件卡片显示不支持。按 ID 定向装载由生命周期的空结果分支落状态，全量装载不逐个遍历
  目标，由加载器自己落记。
- 启动同步把运行时不兼容的插件排除在候选之外。它们装不上，进入同步只会得到一次必然失败的
  安装，并用同步失败状态覆盖不兼容状态；恢复集合还会据此把整个启动同步判为未完成。

该声明是人工标注，覆盖率天然滞后于真实依赖状况，不作为 V3t 依赖兼容性的唯一判据。
