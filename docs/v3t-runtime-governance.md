...
| 启动就绪中位数 | 8.438 s | 7.301 s | 该候选中 V3t 未造成启动退化；不据此承诺所有部署都会更快。 |
| 32 线程纯 Python CPU 探针 | 12,062.9 ops/s | 38,340.8 ops/s | V3t 约为 3.18 倍，证明解释器并行收益；该探针不经过 Rust 开关。 |
| 32 线程直接 Rust ABI 探针 | 37,620.2 ops/s | 47,225.7 ops/s | 两边都直接调用 Rust，用于验证原生 ABI 与并发，不代表 V3 关闭 Rust 的表现。 |
| 单线程解释器探针 | 基线 | 约慢 4.8% | 单线程成本存在但低于 25% 验收阈值，不能用并发收益掩盖后续扩大退化。 |
...
旧候选的 schema 2 smoke 已分别采到 `0.009364s`、`0.010274s`、`0.010685s`，并确认三组业务结果
校验和一致。这是缩短迭代数的采集链验证，且候选 revision 已过期，不能据此得出“Rust 更快”或“Rust
更慢”的产品结论。最终报告必须单独展示三组中位数及 `V3 + Rust / V3 + Python`、
`V3t + Rust / V3 + Rust` 两个比例；任何一组退化都不能被另一组的收益抵销。
...
| V3 + `psycopg3 binary` | 0.92x | 0.58x | 约 0.18x-0.20x |
| V3t + `psycopg3 C` | 0.86x | 0.88x | 约 0.18x-0.20x |

因此标准 V3 保留 `psycopg2`，V3t 为满足 free-threaded ABI 使用 `psycopg3 C`。`psycopg3` 的批量
事务写明显更快，但查询吞吐没有形成全面优势；当前不为统一驱动扩大数据库重构。未来驱动或应用访问
模式变化时，必须重跑真实短事务、批量写、连接池、长事务和迁移场景，不能只用单项微基准改写结论。
...
扩充后的公共 A/B schema 还会记录完整安装包集合及哈希、全部原生 distribution 和 wheel tag、导入
前后 GIL、working set、RSS/PSS/USS、API p50/p95、SQLite 同步/异步结果和 1/8/16/32 线程热点。
最终候选只有在这些新增样本和 PostgreSQL、插件恢复场景一起通过后，才能更新本节为当前发布结论。
