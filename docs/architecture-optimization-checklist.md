...
| 指标 | 当前值 | 解释 |
|---|---:|---|
| 宿主 Python 模块 / 内部依赖边 | 972 / 8,301 | `dependency-baseline.json` 当前快照 |
...
| 全量 mypy 历史债务 | 10,231 / 592 文件 | 识别链 typed plan/result、TMDB、消息、Indexer 与 AniList/Bangumi/IMDb 双 ABI 共用计划继续清除参数、返回值和泛型注解债；strict frontier 当前覆盖 41 个文件 |
| Ruff 历史诊断 | 641 | 低水位门禁通过，但规则集只覆盖 `E4/E7/E9/F/I` |
