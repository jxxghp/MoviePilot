...
| 全量 mypy 历史债务 | 10,100 / 592 文件 | 识别链 typed plan/result、TMDB API、消息、Indexer、目录源与 Recommend 双 ABI 共用计划继续清除参数、返回值和泛型注解债；strict frontier 当前覆盖 41 个文件 |
| Ruff 历史诊断 | 640 | 低水位门禁通过，但规则集只覆盖 `E4/E7/E9/F/I` |
